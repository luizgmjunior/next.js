...
impl<B: BackingStorage> TurboTasksBackendInner<B> {
    ...
    fn snapshot_and_persist(
        ...
        {
            // Tasks were already removed from persisted_task_cache_log (consumed
            // by save_snapshot), so a future snapshot would not re-persist them —
            // returning an error signals to the caller that further persist
            // attempts would corrupt the task graph in storage.
